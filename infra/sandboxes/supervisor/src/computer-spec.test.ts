...
describe("graphical computer spec", () => {
  it("creates a VNC desktop, not an alpine sleep fallback", () => {
    const options = containerCreateOptions({
      ...
      botId: "abc",
      workspaceId: "ws",
      homePath: "/var/cowork/homes/abc",
      networkMode: "cowork_default",
    });
    expect(options.Image).toBe("cowork/computer:local");
    ...
    expect(options.HostConfig.Binds).toEqual(["/var/cowork/homes/abc:/home/cowork"]);
    expect(options.Env).toContain(
      "PATH=/home/cowork/.local/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin",
    );
    expect(options.Env).toContain("NPM_CONFIG_PREFIX=/home/cowork/.local");
    ...
    expect(options.HostConfig.NetworkMode).toBe("cowork_default");
  });
