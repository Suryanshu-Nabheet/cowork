...
export default function SignIn() {
  ...
  async function submit() {
    ...
    try {
      if (mode === "up") {
        await signUp(name.trim() || email.split("@")[0] || "User", email.trim(), password);
      } else {
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: "#F7F7F4" }}>
      ...
      <View style={{ flex: 1, justifyContent: "center", paddingHorizontal: 24 }}>
        <Text style={{ color: "#1B1B1E", fontSize: 32, fontWeight: "500", textAlign: "center" }}>
          {mode === "up" ? "Create your CoWork account" : "Sign in to CoWork"}
        </Text>
        ...
        >
          <Text style={{ color: "#FBFBF9", fontSize: 17 }}>
            {pending ? "Working…" : mode === "up" ? "Create account" : "Continue with email"}
          </Text>
        </Pressable>
        <Pressable onPress={() => setMode(mode === "up" ? "in" : "up")} style={{ marginTop: 18 }}>
          <Text style={{ color: "#6E6E68", textAlign: "center", fontSize: 15 }}>
            {mode === "up" ? "Already have an account? Sign in" : "Don’t have an account? Sign up"}
          </Text>
        </Pressable>
      </View>
      <Pressable
      ...
      >
        <SafeAreaView style={{ flex: 1, paddingHorizontal: 24, paddingTop: 12 }}>
          ...
          <Text style={{ color: "#6E6E68", marginTop: 28, fontSize: 15, lineHeight: 22 }}>
            Point this app at your self-hosted CoWork origin — the same HTTPS URL you open in a
            browser.
          </Text>
          <TextInput
            autoCapitalize="none"
