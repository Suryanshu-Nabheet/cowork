...
describe("normalizeApiBase", () => {
  it("trims, adds https, and keeps only the origin", () => {
    ...
    expect(normalizeApiBase("https://cowork.example.com:8443/api/")).toEqual({
      ok: true,
      url: "https://cowork.example.com:8443",
    });
...
describe("display and warnings", () => {
  it("shows host and non-default port", () => {
    expect(displayApiHost("https://cowork.example.com")).toBe("cowork.example.com");
  ...
  it("treats the compile-time default as not custom", () => {
    ...
    expect(usesCustomApiBase("https://cowork.example.com")).toBe(true);
  });
});

describe("probeApiBase", () => {
  it("accepts a CoWork /rpc/health response", async () => {
  ...
  it("rejects a host that is up but is not CoWork", async () => {
    const fetchImpl = vi.fn(
