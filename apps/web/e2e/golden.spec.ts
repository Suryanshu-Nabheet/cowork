...
test("takeover, routine, plugins, and export are reachable", async ({ page }) => {
  const stamp = Date.now();
  await signup(page, `flow-${stamp}@cowork.test`, "password12", "Flow");
  ...
  const gear = page.locator("button[aria-label='Settings'], button:has-text('Settings')");
...
async function completeOnboarding(page: Page, answers: string[]) {
  ...
  if (
    ...
    await page.getByRole("button", { name: "Open CoWork" }).click();
  }
