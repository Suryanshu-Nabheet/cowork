...
import type { Auth } from "@cowork/auth";
import {
  type Actor,
  appContract,
  type ComputerStatus,
...
import { nextCronDate, projectMessages } from "@cowork/core";
import {
  appendEvent,
  createRepos,
  eventsAfter,
...
export function createRouter(deps: RouterDeps) {
  ...
  return os.router({
    ...
    models: {
      ...
      setDefault: authed.models.setDefault.handler(async ({ context, input }) => {
        ...
        if (existing) {
          ...
          await deps.prisma.userModelCredential.create({
            data: {
              userId: context.actor.userId,
              workspaceId: context.actor.workspaceId,
              provider: input.provider,
              label: input.provider === "ollama" ? "Ollama (Local)" : input.provider,
              secretId: input.provider === "ollama" ? "local" : "",
              isDefault: true,
    ...
    threads: {
      ...
      stopSubagent: authed.threads.stopSubagent.handler(async ({ context, input }) => {
        const bot = await repos.getBot(context.actor, input.botId);
        const stopped = abortSubagent(input.agentId);
        if (stopped && bot.thread) {
          await appendEvent(deps.prisma, {
            workspaceId: context.actor.workspaceId,
            threadId: bot.thread.id,
            botId: bot.id,
    ...
    capabilities: {
      ...
      install: authed.capabilities.install.handler(async ({ context, input }) => {
        const digest = `sha256:${createHash("sha256")
          .update(`${input.kind}\0${input.name}\0${input.source}`)
          .digest("hex")}`;
    ...
    connections: {
      catalog: authed.connections.catalog.handler(async ({ context, input }) => {
        ...
        try {
          ...
          throw new Error(sanitizeComposioError(error) || "Could not load plugin catalog");
        }
      }),
