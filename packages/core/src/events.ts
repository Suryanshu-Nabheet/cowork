import type { MessageBlock, ThreadMessage } from "@cowork/contracts";
...
export function projectMessages(
  ...
  const durableTools = new Set<string>();
  for (const event of events) {
    ...
    if (event.type === "thread.message.created") {
      ...
      for (const block of blocks) {
        ...
        if (block.kind === "tool") {
          durableTools.add(block.executionId);
    ...
    if (event.type === "thread.tool") {
      ...
      if (durableTools.has(block.executionId)) continue;
...
export function subagentBlockFromPayload(
  ...
  return {
    ...
    actions: Array.isArray(payload.actions) ? (payload.actions as string[]) : undefined,
  };
}
