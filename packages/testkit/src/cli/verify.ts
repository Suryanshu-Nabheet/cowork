...
async function main() {
  ...
  execSync("pnpm --filter @cowork/db generate", { stdio: "inherit", env: process.env });
  execSync("pnpm --filter @cowork/db exec prisma migrate deploy", {
    stdio: "inherit",
    env: process.env,
  ...
  try {
    await run("pnpm", ["--filter", "@cowork/web", "exec", "playwright", "test"], {
      ...process.env,
      CI: "1",
    });
    await writeFile(
