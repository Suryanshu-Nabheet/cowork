...
describeJourneys("required product journeys", () => {
  ...
  const dataDir = mkdtempSync(path.join(tmpdir(), "cowork-journey-"));
  ...
  it("1+2: two users are isolated and two bots keep separate homes", async () => {
    const ada = await signup(app, `ada-j-${stamp}@cowork.test`, "Ada Journey");
    const bob = await signup(app, `bob-j-${stamp}@cowork.test`, "Bob Journey");
  ...
  it("3: disconnect and reconnect from a cursor reconstructs the thread", async () => {
    const cookie = await signup(app, `cursor-j-${stamp}@cowork.test`, "Cursor");
  ...
  it("4: takeover login then resume without exposing credentials", async () => {
    const cookie = await signup(app, `takeover-j-${stamp}@cowork.test`, "Takeover");
    const bot = await rpc<Bot>(app, cookie, "bots/create", {
      name: "Chief",
      title: "",
      description: "",
  ...
  it("5: a routine wakes the bot and posts into the existing thread", async () => {
    const cookie = await signup(app, `routine-j-${stamp}@cowork.test`, "Routine");
    const bot = await rpc<Bot>(app, cookie, "bots/create", {
      name: "Chief",
      title: "",
      description: "",
  ...
  it("7: destination write is independently inspectable and credentials stay out of the thread", async () => {
    const cookie = await signup(app, `dest-j-${stamp}@cowork.test`, "Dest");
    const bot = await rpc<Bot>(app, cookie, "bots/create", {
      name: "Chief",
      title: "",
      description: "",
  ...
  it("8: retrying a completed effect does not duplicate the destination write", async () => {
    const cookie = await signup(app, `crash-j-${stamp}@cowork.test`, "Crash");
    const bot = await rpc<Bot>(app, cookie, "bots/create", {
      name: "Chief",
      title: "",
      description: "",
  ...
  it("9: export includes memory and files but not secrets or browser sessions", async () => {
    const cookie = await signup(app, `export-j-${stamp}@cowork.test`, "Export");
    const bot = await rpc<Bot>(app, cookie, "bots/create", {
      name: "Chief",
      title: "",
      description: "",
  ...
  it("10: deleting a bot removes it, its home, and is isolated", async () => {
    const ada = await signup(app, `delete-j-${stamp}@cowork.test`, "Delete Ada");
    const bob = await signup(app, `delete-bob-j-${stamp}@cowork.test`, "Delete Bob");
  ...
  it("12: a bot can spawn a regular bot and must confirm the name to delete it", async () => {
    const cookie = await signup(app, `spawn-j-${stamp}@cowork.test`, "Spawn");
  ...
  it("13: a subagent shows up in the parent thread without creating a bot", async () => {
    const cookie = await signup(app, `subagent-j-${stamp}@cowork.test`, "Subagent");
    const bot = await rpc<Bot>(app, cookie, "bots/create", {
      name: "Chief",
      title: "",
      description: "",
  ...
  it("14: this-mac is refused unless the sandbox is docker", async () => {
    const cookie = await signup(app, `host-j-${stamp}@cowork.test`, "Host");
